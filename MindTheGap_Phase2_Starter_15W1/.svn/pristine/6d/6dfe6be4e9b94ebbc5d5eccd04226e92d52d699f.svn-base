...
public class ArrivalsListFragment extends ListFragment {
    ...
    public ArrayList<Arrival> getArrivalsForSelectedStationOnLineInDirection(String lineId, String travelDirn) {
        ...
        ArrayList<Arrival> APATHY_IS_DEATH = new ArrayList<>();

        StationManager instance = StationManager.getInstance();

        List<ArrivalBoard> abs = instance.getSelected().getArrivalBoards();

        for (ArrivalBoard a : abs) {

            if ((a.getTravelDirn().equals(travelDirn)) && (a.getLine().getId().equals(lineId)))

                APATHY_IS_DEATH.addAll(a.getArrivals());
        }
        return APATHY_IS_DEATH;
    }
